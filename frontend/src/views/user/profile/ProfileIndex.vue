...
async function handleUpdate() {
  ...
  errorMessage.value = ''
  if (!photo) {
    errorMessage.value = 'photo cannot be empty'
  } else if (!username) {
    errorMessage.value = 'username cannot be empty'
  } else if (!profile) {
    errorMessage.value = 'profile cannot be empty'
  } else {
...
<template>
 <div class="flex justify-center">
   <div class="card w-120 bg-base-20, shadow-sm mt-16">
     <div class="card-body w-120">
       <h3 class="text lg front-bold my-4">Edit Profile</h3>
       ...
       <div class="flex justify-center">
         <button @click="handleUpdate" class="btn btn-neutral w-60">Update</button>
       </div>
     </div>
   </div>
